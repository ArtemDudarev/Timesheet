...
@pytest.fixture
def manager_token():
    return create_test_token(str(MANAGER_USER_ID), ["Менеджер", "Сотрудник"], _MANAGER_PERMISSIONS)


@pytest.fixture
def admin_token():
    return create_test_token(str(MANAGER_USER_ID), ["Администратор", "Сотрудник"], _ADMIN_PERMISSIONS)
