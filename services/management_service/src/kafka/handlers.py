...
async def handle_employee_updated(payload: dict[str, Any], producer=None) -> None:
    ...
    async with async_session_maker() as session:
        ...
        # Реплика назначений для guard'а на удаление справочников
        if "department_id" in employee_data:
            raw = employee_data["department_id"]
            employee.department_id = UUID(raw) if raw else None
        if "grade_id" in employee_data:
            raw = employee_data["grade_id"]
            employee.grade_id = UUID(raw) if raw else None
        await session.commit()
