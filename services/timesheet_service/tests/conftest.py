...
OUTSIDER_PERIOD_ID = uuid.UUID("bbbbbbbb-0000-0000-0000-000000000002")


_MANAGER_PERMISSIONS = [
    "timesheet:read_any", "timesheet:edit_any", "timesheet:close_period",
    "timesheet:approve_period", "overtime:approve", "summary:read_any",
]

_TEAMLEAD_PERMISSIONS = [
    "timesheet:read_team", "timesheet:approve_period_team",
    "overtime:approve_team", "summary:read_team",
]
...
async def client(db_session):
    ...
    with (
        ...
        patch("src.main.seed_entry_types", new_callable=AsyncMock),
        patch("src.main.seed_production_calendar", new_callable=AsyncMock),
        patch("src.main.seed_existing_employee_periods", new_callable=AsyncMock),
...
async def test_outsider(db_session):
    # Сотрудник без лида — не в команде тимлида
...
@pytest_asyncio.fixture
async def test_period(db_session, test_employee):
...
@pytest_asyncio.fixture
async def test_outsider_period(db_session, test_outsider):
    period = TimesheetPeriod(
        id=OUTSIDER_PERIOD_ID,
...
@pytest.fixture
def teamlead_token():
    return create_test_token(str(TEAMLEAD_ID), ["Тимлид", "Сотрудник"], _TEAMLEAD_PERMISSIONS)
